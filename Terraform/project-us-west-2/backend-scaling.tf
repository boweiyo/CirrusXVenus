...
resource "aws_launch_template" "bowei-backend-80" {
  name_prefix = "bowei-backend-80"
  # us-west-2
...
resource "aws_autoscaling_group" "bowei-backend-asg" {
  ...
  launch_template {
    id      = aws_launch_template.bowei-backend-80.id
    version = "$Default"
  }
  target_group_arns = [aws_lb_target_group.bowei-backend-tg-80.arn]
}
resource "aws_autoscaling_policy" "bowei-backend-asp" {
  name                   = "bowei-backend-asp"
  autoscaling_group_name = aws_autoscaling_group.bowei-backend-asg.name
...
resource "aws_lb" "bowei-backend-lb" {
  name               = "bowei-backend-lb"
  internal           = false
...
resource "aws_lb_listener" "bowei-backend-lb-listener-80" {
  load_balancer_arn = aws_lb.bowei-backend-lb.arn
  port              = "80"
  protocol          = "TCP"
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.bowei-backend-tg-80.arn
  }
}
resource "aws_lb_target_group" "bowei-backend-tg-80" {
  name     = "bowei-backend-tg-80"
  port     = 80
  protocol = "TCP"
  vpc_id   = aws_vpc.bowei-vpc.id
}
