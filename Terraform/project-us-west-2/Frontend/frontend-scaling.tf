...
data "aws_subnet_ids" "bowei-frontend-subnets" {
  vpc_id = data.aws_vpc.bowei-vpc.id
}
...
resource "aws_autoscaling_group" "bowei-frontend-asg" {
  ...
  vpc_zone_identifier       = [data.aws_subnet.bowei-vpc-pb-2a.id, data.aws_subnet.bowei-vpc-pb-2b.id]
...
resource "aws_lb" "bowei-frontend-lb" {
  ...
  subnets = [data.aws_subnet.bowei-vpc-pb-2a.id, data.aws_subnet.bowei-vpc-pb-2b.id]
}
